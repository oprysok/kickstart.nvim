-- Database management via vim-dadbod (+ UI and completion).
-- vim-dadbod-completion is wired into blink.cmp via the `dadbod` source in init.lua.
...
-- Helpers + keymaps for viewing DB result rows. Wrapped in a `do` block so the
-- locals stay scoped; the keymap closures keep them alive as upvalues.
do
  ...
  local function show_popup(content_lines, filetype, context)
    ...
    if context and context.original_query and context.db_info then
      vim.keymap.set('n', '<leader>dj', function()
        ...
        local db_url_ctx = context.db_info.db_url or context.db_info
        local parsed = vim.fn['db#url#parse'](db_url_ctx)
